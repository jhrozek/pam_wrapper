...
#define PWRAP_DLIST_ADD(list,item) do { \
	if (!(list)) { \
		(item)->prev	= NULL; \
		(item)->next	= NULL; \
		(list)		= (item); \
...
#define PWRAP_DLIST_REMOVE(list,item) do { \
	if ((list) == (item)) { \
		(list)		= (item)->next; \
		if (list) { \
			(list)->prev	= NULL; \
...
# define PWRAP_LOG(...)
#else /* NDEBUG */
static void pwrap_log(enum pwrap_dbglvl_e dbglvl, const char *format, ...) PRINTF_ATTRIBUTE(2, 3);
# define PWRAP_LOG(dbglvl, ...) pwrap_log((dbglvl), __VA_ARGS__)

static void pwrap_log(enum pwrap_dbglvl_e dbglvl, const char *format, ...)
{
	char buffer[1024];
	va_list va;
	const char *d;
 ...
 * IMPORTANT
 *
 * Functions expeciall from libpam need to be loaded individually, you can't load
