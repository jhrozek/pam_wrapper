...
static int p_rmdirs_rec(const char *path, int parent_fd)
{
	DIR *d;
	struct dirent *dp;
	struct stat sb;
	...
	while ((dp = readdir(d)) != NULL) {
		...
		if (S_ISDIR(sb.st_mode)) {
			rc = p_rmdirs_rec(dp->d_name, path_fd);
		} else {
...
static int p_rmdirs(const char *path)
{
	/* If path is absolute, openat would ignore parent_fd anyway.
	 * If it's relative, start from cwd
	 */
	return p_rmdirs_rec(path, AT_FDCWD);
}
